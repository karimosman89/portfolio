...
- **Live Playground — working AI proof-of-concepts** (not slideware) that visitors can actually run against real backend endpoints:
  - **RAG Assistant** — grounded Q&A over Karim's CV (`POST /api/chat`).
  - **Long-Context** — live document chunking (configurable size/overlap) + LLM summarization (`POST /api/summarize-chunk`).
  - **Multimodal Vision** — upload an image and get a structured Gemini analysis (`POST /api/gemini/analyze-multimodal`).
  - Each demo **degrades gracefully into a clearly-labelled "demo mode"** when `GEMINI_API_KEY` is absent, so the POCs stay functional in any environment.
