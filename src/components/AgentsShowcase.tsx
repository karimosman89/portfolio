    ...
    <div className="overflow-x-auto rounded-2xl glass p-6">
      <svg viewBox="0 0 700 200" className="mx-auto w-full min-w-[640px]" role="img" aria-label="Agentic workflow diagram">
        ...
        {edges.map(([a, b], i) => {
          ...
          return (
            <path key={i}
              d={`M ${na.x + 34} ${na.y} C ${(na.x + nb.x) / 2} ${na.y}, ${(na.x + nb.x) / 2} ${nb.y}, ${nb.x - 34} ${nb.y}`}
              fill="none" stroke="url(#edgeGrad)" strokeWidth="1.5" className="flow-line" />
          );
        })}
        {nodes.map((n) => (
          <g key={n.id}>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {AGENT_CAPABILITIES.map((c, i) => (
          <Reveal key={c.id} delay={(i % 3) * 0.06}>
            <div className="group flex h-full items-start gap-4 rounded-2xl glass p-5 glow-hover">
              ...
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {c.tags.map((t) => (
                    <span key={t} className="rounded-md bg-[rgb(var(--border)/0.06)] px-2 py-0.5 text-[10px] font-mono text-[var(--text-muted)]">{t}</span>
                  ))}
                </div>
              </div>
            </div>
